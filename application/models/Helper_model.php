...
class Helper_model extends CI_Model {
	/*
	http://github.com/thedijje/cihelper
	http://thedijje.ic@gmail.com
	http://twittter.com/thedijje
	...
	*/
		...
		*/
		...
		$target_dir = $path;
		$time=time();
		$target_file = $target_dir . $time."_".str_replace(" ","-",basename($_FILES[$name]["name"]));
	...
	*/
	...
	public function send_formatted_mail($data){
		...
		$this->email->from($this->settings_model->get_settings('email'), $this->settings_model->get_settings('sitename'));
		$this->email->subject($data['subject']." - ".$this->settings_model->get_settings('sitename'));
	...
	*/
	...
	public function search($table,$key_set){
	
	
	if(!empty($key_set)){
			
			foreach ($key_set as $key => $value){
			$query=$this->db->or_like($key, $value);
			}
		
		}else{
		log_message('error','No key defined for search');
		return FALSE;
		}
		
		$query=$this->db->get($table);
		if(!$query){
		log_message('error','Searching error : '.mysql_query());
		return FALSE;	
		}else{
		return $query->result();
		}
